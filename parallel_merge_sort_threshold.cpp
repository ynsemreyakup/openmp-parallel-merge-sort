...
void sequential_merge_sort(
    ...
    constexpr int small_range_limit = 64;

    if (right - left <= small_range_limit) {
...
void parallel_merge_sort(
    ...
    constexpr int small_range_limit = 64;
    constexpr int task_range_limit = 1 << 15;
    ...
    if (remaining_task_depth > 0 && right - left >= task_range_limit) {
